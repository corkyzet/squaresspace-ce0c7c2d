...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Try today first, then look ahead up to 7 days for upcoming games
    const today = new Date();
    let allEvents: any[] = [];

    // Fetch today
    const todayStr = formatDate(today);
    allEvents = await fetchGamesForDate(todayStr);

    // If no games today (excluding First Four), look ahead
    const nonFirstFour = allEvents.filter((e: any) => {
    ...
    if (nonFirstFour.length === 0) {
      ...
      for (let i = 1; i <= 7; i++) {
        ...
        const futureEvents = await fetchGamesForDate(futureDateStr);
